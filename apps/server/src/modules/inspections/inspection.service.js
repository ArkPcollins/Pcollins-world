...
export class InspectionService {
    ...
    async bookInspection(userId, propertyId, scheduledDate, message) {
        ...
        if (existing)
            throw new Error("You already have a pending inspection for this property");
        ...
        if (property.landlordId) {
            await this.notificationService.create(property.landlordId.toString(), "New Inspection Request", `Someone wants to inspect your property: ${property.title}`);
        }
        await inspection.populate("propertyId userId");
        return inspection;
    }
    async updateStatus(inspectionId, status, adminNotes) {
        ...
        await inspection.save();
        // Notify user
        await this.notificationService.create(inspection?.userId?.toString() ?? "", "Inspection Status Updated", `Your inspection request has been ${status.toLowerCase()}`);
        return inspection;
    }
    async getUserInspections(userId, page = 1, limit = 10) {
    ...
    async getPropertyInspections(propertyId, page = 1, limit = 10) {
